...
function MyPostsPage() {
    ...
    useEffect(() => {
        axios.get('https://localhost:7287/api/posts')
            .then(response => {
                // handle success
                //response.data.filter(p => p.authorName == username)
        ...
        <>
            ...
            <div className='posts_page'>
                ...
                <div className='posts_container'>
                    {posts?.filter(p => p.authorName == username).map(post => <a className='post_link' key={post.postId} href={`/posts/${post.postId}`}><Post authorName={post.authorName} title={post.title} description={post.description} dateTime={post.dateTime} /></a>)}
                </div>
